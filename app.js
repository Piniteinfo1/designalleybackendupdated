...
// Route to handle POST request for designer signup with multer configuration
app.post("/designer/signup/",jsonParser,async(request,response)=>{
  ...
  const {name,address,email,area,budget,bankName,accountNumber,branch,ifscCode,PhoneNumber,logo,Gender,gstNumber,Category}=desigenerDetails
 ...
 if(dbResponse===undefined){
  const designerSignupQuery=`INSERT INTO interior_designer_details (desigener_name,email_id,phone_number,area,logo,budget,bank_name,account_number,branch,ifsc_code,address,number_of_posts,number_of_followers,number_of_following,Gender,gstNumber,Category)
  VALUES('${name}','${email}','${PhoneNumber}','${area}','${logo}','${budget}','${bankName}','${accountNumber}','${branch}','${ifscCode}','${address}',0,0,0,'${Gender}','${gstNumber}','${Category}');`;
  ...
  const fileName=Date.now()+"_"+request.files.logo.name
...
// Edit post
app.put("/editpost", jwtAuthenticateToken, jsonParser, async (request, response) => {
  const { postId, caption, tags } = request.body;

  if (!postId || !caption || !tags) {
  ...
  try {
    const updateQuery = `
      ...
      WHERE postId = ?
    `;
    const dbResponse = await db.run(updateQuery, [caption, tags, postId]);
    response.status(200).json({ success: true, changes: dbResponse.changes });
  } catch (error) {
...
app.get('/IndividualDesignerProfile/:id', jwtAuthenticateToken, async (req, res) => {
  const designer_id = req.params.id;

  try {
   
    const selectQuery = 'SELECT logo, desigener_name,number_of_followers,number_of_following ,number_of_posts,designer_id FROM interior_designer_details WHERE designer_id = ?';
    
    
    const dbResponse = await db.get(selectQuery, [designer_id]);
    ...
    res.json({
      logo: dbResponse.logo, 
      desigener_name: dbResponse.desigener_name, 
      number_of_followers:dbResponse.number_of_followers,
      number_of_following:dbResponse.number_of_following,
      number_of_posts:dbResponse.number_of_posts,
      designer_id:dbResponse.designer_id,    
...
//Individual Designer Profile-Post count

app.get("/designerPostsCount/:designerId", jwtAuthenticateToken, jsonParser, async (request, response) => {
  const { designerId } = request.params;
  
  
  try {
    
      const getPostsCount = `SELECT COUNT(*) AS postCount FROM designerPost WHERE designerId=${designerId}`;
      const postCountResponse = await db.get(getPostsCount);
      
     
      const getAllPosts = `SELECT * FROM designerPost WHERE designerId=${designerId} ORDER BY createdAt DESC`;
      const postResponse = await db.all(getAllPosts);
      
    
      const designerPostDetails = JSON.stringify({
          postResponse,
          postCountResponse
      });
      ...
      console.error('Error fetching designer post details:', error);
...
app.get("/designerSelectedPost/:postId", async (req, res) => {
  const { postId } = req.params;
  const getPost = `SELECT * FROM designerPost WHERE postId=${postId};`;
  try {
    const postResponse = await db.all(getPost);
    if (postResponse.length > 0) {
      res.send(postResponse[0]); 
    } else {
      res.status(404).send({ message: 'Post is not there' });
    }
  } catch (error) {
...
app.post("/likesCount", jwtAuthenticateToken, jsonParser, async (request, response) => {
  const { postId } = request.body;
  const { userNumber } = request;
  const likedUserIdQuery = `SELECT designer_id FROM interior_designer_details WHERE phone_number = '${userNumber}';`;
  
  try {
  
    const postResponse = await db.all(likedUserIdQuery);
    const designerId = postResponse[0].designer_id;
    
   
    const checkLikeQuery = `
      SELECT * FROM likes
      WHERE postId = ${postId} AND userId = ${designerId};`;

    const existingLike = await db.all(checkLikeQuery);
    ...
    if (existingLike.length === 0) {
      ...
      await db.run(insertLikesQuery);

  
      const updateLikesCountQuery = `
        UPDATE designerPost
        SET likes = (SELECT COUNT(*) FROM likes WHERE postId = ${postId})
        WHERE postId = ${postId};`;

      await db.run(updateLikesCountQuery);
    } else {
    ...
    const countLikesQuery = `SELECT likes FROM designerPost WHERE postId = ${postId};`;
    const { likes } = await db.get(countLikesQuery);

    response.json({ likeCount: likes });
  } catch (error) {
    console.error("Error:", error);
...
// Individual Designer Profile -Follow/Following 


app.post('/followDesigner/:designerId', jwtAuthenticateToken, async (req, res) => {
    const { designerId } = req.params;
    const {userNumber}=req
     try {
        const checkDesignerQuery = `SELECT * FROM interior_designer_details WHERE designer_id = ?`;
        const checkDesignerResult = await db.get(checkDesignerQuery, [designerId]);

        if (!checkDesignerResult) {
            return res.status(404).json({ message: 'Designer not found' });
        }

        const updateDesignerFollowersQuery = `UPDATE interior_designer_details SET number_of_followers = number_of_followers + 1 WHERE designer_id = ?`;
        await db.run(updateDesignerFollowersQuery, [designerId]);

        const updateUserFollowingQuery = `UPDATE interior_designer_details SET number_of_following = number_of_following + 1 WHERE phone_number = ?`;
        await db.run(updateUserFollowingQuery, [userNumber]);

        res.status(200).json({ message: 'Successfully followed the designer' });
        console.log(designerId);
        console.log(userNumber);
        
    } catch (error) {
        console.error('Error following designer:', error);
        res.status(500).json({ message: 'Failed to follow the designer' });
    }
});

app.delete('/unfollowDesigner/:designerId', jwtAuthenticateToken, async (req, res) => {
  const { designerId } = req.params;
  const { userNumber } = req;
  ...
  try {
      ...
      res.status(200).json({ message: 'Successfully unfollowed the designer' });
  } catch (error) {
      console.error('Error unfollowing designer:', error);
      res.status(500).json({ message: 'Failed to unfollow the designer' });
  }
});


//app.post("/IndividualDesignerprofilefollow", jwtAuthenticateToken,jsonParser,async(request,response) =>{
//const {designer_id} =request.body;
//const {userNumber}=request;
//const IndividualDesignerProfile =`SELECT designer_id FROM interior_designer_details WHERE phone_number ='${userNumber}'`;
...
app.delete("/UnLike", jwtAuthenticateToken, jsonParser, async (request, response) => {
  const { postId } = request.body;
  const { userNumber } = request;
  const likedUserIdQuery = `SELECT designer_id FROM interior_designer_details WHERE phone_number = '${userNumber}';`;
  ...
  try {
    ...
    const deleteResult = await db.run(deleteLikeQuery);
  const updateLikesCountQuery = `UPDATE designerPost SET likes = (SELECT COUNT(*) FROM likes WHERE postId =${postId}) WHERE postId =${postId};`;
await db.run(updateLikesCountQuery);
if (deleteResult.changes === 0) {
      response.status(400).json({ error: "You have not liked this post." });
      return;
    }

   
    const countLikesQuery = `SELECT COUNT(*) as likeCount FROM likes WHERE postId = ${postId};`;
    const likesCount = await db.get(countLikesQuery);

    response.json({ likeCount: likesCount.likeCount });
  } catch (error) {
    console.error("Error:", error);
    response.status(500).json({ error: "Internal Server Error" });
  }
...
app.get ("/getLikedPost", jwtAuthenticateToken , jsonParser, async(request, response) =>{
  ...
  const designerResponse =await db.get(getDesigner);
  const DesignerId1 =designerResponse.designer_id;
  const getallLikedPosts = `SELECT postId FROM likes WHERE userId =${DesignerId1} ORDER BY LikeId DESC;`;
  const postResponse =await db.all(getallLikedPosts);
  
  const postResponseLength=postResponse.length-1
  postResponse.forEach(element=> {
    postArray.push(element.postId);
  });
  const LikedPostData =`SELECT * FROM designerPost WHERE postId IN (${postArray});`;
  const dbResponse =await db.all(LikedPostData)
  response.send(dbResponse)
})
...
app.delete('/deletepdf', async (req, res) => {
  const { filename, projectId } = req.query;
  const { projectName } = await fetchEstimatesFromProject(projectId);
  const filePath = path.join(__dirname, 'Projects', projectName, 'estimates', filename);
  ...
  try {
    ...
    fs.unlink(filePath, async (err) => {
      ...
      db.run(deletePdfQuery, [filename], async (err) => {
        if (err) {
          console.error('Error deleting PDF data from database:', err.message);
          return res.status(500).json({ error: 'Failed to delete PDF data from database' });
        }
...
app.get('/api/fetchpdfss', async (req, res) => {
  try {
    const selectQuery = "SELECT projectId, title FROM projects;";
    const projects = await db.all(selectQuery);
    ...
    for (let project of projects) {
      ...
      try {
        const files = await fs.promises.readdir(projectFolderPath);
        const projectPdfFiles = await Promise.all(files.map(async (file) => {
          const filePath = path.join(projectFolderPath, file);
          const stats = await fs.promises.stat(filePath);
          return {
            filename: file,
        ...
        pdfFiles.push(...projectPdfFiles);
      } catch (error) {
        console.error(`Error reading files for project ${projectName}:`, error);
      }
    }
...
app.put('/updateProfile', upload.single('logo'), async (req, res) => {
  const multer = require('multer');
const path = require('path');

const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, 'uploads')); // Adjust destination folder as needed
  },
  filename: function (req, file, cb) {
    cb(null, file.originalname); // Keep original filename; adjust as needed
  }
});
...
const upload = multer({ storage: storage });
  ...
  try {
    const { desigener_name, email_id, phone_number, Gender, designer_id, } = req.body;
    const logo = req.file ? req.file.filename : null; // Assuming logo field stores filename in database

    // Update query to update profile details in database
    const updateProfileQuery = `
      UPDATE interior_designer_details 
      SET desigener_name = ?, email_id = ?, phone_number = ?, logo = ?, Gender = ?
      WHERE designer_id = ?
    `;
    
    const params = [desigener_name, email_id, phone_number, logo, Gender, designer_id];

    await new Promise((resolve, reject) => {
      db.run(updateProfileQuery, params, function(err) {
        if (err) {
          reject(err);
        } else {
          resolve(this.lastID); // Return the ID of the updated profile, if needed
        }
      });
    });

    // Fetch updated profile data to send back in response
    const updatedProfile = await new Promise((resolve, reject) => {
      db.get('SELECT * FROM interior_designer_details WHERE designer_id = ?', [designer_id], (err, row) => {
        if (err) {
          reject(err);
        } else {
          resolve(row);
